...
public class GildedRose
{
    ...
    public void UpdateQuality()
    {
        foreach (var item in _items)
        {
            ...
            if (item.Name != "Sulfuras, Hand of Ragnaros" && item.Name != "Backstage passes to a TAFKAL80ETC concert" && item.Name != "Aged Brie" && item.Quality > 0 && item.SellIn < 0) item.Quality = item.Quality - 1;

            if (item.Name == "Backstage passes to a TAFKAL80ETC concert" && item.SellIn < 0) item.Quality = item.Quality - item.Quality;

            if (item.Name == "Aged Brie" && item.Quality < 50 && item.SellIn < 0) item.Quality = item.Quality + 1;
        }
    }
